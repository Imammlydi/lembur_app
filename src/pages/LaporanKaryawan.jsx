...
const API_URL2 = "https://server-reports.kencang.id/report-app4/public/api";
...
const tanggalSekarang = new Date().toLocaleDateString('id-ID', options);
    ...
    useEffect(() => {
        ...
        console.log("Tahun:", tahun,'--',tanggalSekarang);
        console.log("total mebur malam:", lemburMalam);
        console.log("totalmlembur isiang :", lemburSm,'--',tanggalSekarang);
        localStorage.setItem("bulan", selectedMonth),
            localStorage.setItem("tahun", selectedYear);
    }, [token, bulan, tahun,tanggalSekarang,lemburMalam,lemburSm]);
    ...
    const fetchKaryawan = async () => {
        try {
            const response = await axios.get(
                `${API_URL2}/karyawan-date?bulan=${selectedMonth}&tahun=${selectedYear}`,
                {
                    headers: {
    ...
    const fetchLemburMalam = async () => {
        try {
            const response = await axios.get(
                `${API_URL2}/lembur-malam-karyawan-date-all?bulan=${selectedMonth}&tahun=${selectedYear}`,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
            ...
            setLemburMalam(response.data.jumlah_lembur
                );
            console.log(
                response.data.jumlah_lembur,
                "--------"
            );
    ...
    const fetchLemburSm = async () => {
        try {
            const response = await axios.get(
                `${API_URL2}/lembur-sm-karyawan-date-all?bulan=${selectedMonth}&tahun=${selectedYear}`,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
            ...
            setLemburSm(response.data.jumlah_lembur);
            console.log(response.data, "--------");
            ...
            <>
                <div className="my-4 rounded-lg border border-graydisable p-2 shadow-lg">
                    ...
                    <button
                        ...
                        onClick={()=>{fetchKaryawan(),fetchLemburMalam(),fetchLemburSm()}}
                    >
                        Filter
                    </button>
                </div>
    ...
    return (
        <div className="container mx-auto p-4">
            ...
            <div className="rounded-lg border-2 border-green1 p-2 shadow-md grid lg:grid-cols-4 grid-cols-1 md:grid-cols-4 gap-2">
                    <div className=" font-Bitter text-lg ">
                        {" "}
                        Bulan : 
                        {/* <p className="font-bold">{loading == true ?     <ReactLoading
                        type={"spin"}
                        color={"#0388fc"}
                        height={22}
                        width={22}
                    ...
                    <div className=" font-Bitter text-lg ">
                        ...
                        width={22}
                    /> :lemburMalam}</p> */}
                    <p className="font-bold">{lemburMalam}</p>
                    </div>
                    <div className=" font-Bitter text-lg ">
                    Total semua Lembur Siang:
                         {/* <p className="font-bold">{loading == true ?     <ReactLoading
                        type={"spin"}
                        color={"#0388fc"}
                        height={22}
            ...
            <div className="grid grid-cols-1 gap-4 px-4 pt-8 md:grid-cols-3  md:px-8 lg:max-w-7xl lg:grid-cols-2">
                <div>
                    <div className="mb-4 hidden rounded-lg border-2 border-graydisable2 p-3">
                        <input
                            type="text"
                            placeholder="Nama Karyawan"
                            className="border-gray-300 mr-2 mb-2 rounded-lg border p-2"
                            value={namaKaryawan}
                    ...
                    <div className="-my-2 hidden overflow-x-auto sm:-mx-6 lg:-mx-8">
            ...
            <div>
                <div className="mt-5 mb-32 grid grid-cols-1 gap-2 rounded-lg md:grid-cols-2 lg:grid-cols-3">
                    {searchResults.length > 0
                        ? searchResults.map((item, index) => (
                              <div
                                  className="rounded-lg border border-graydisable p-2 shadow-lg"
                                  key={item.id}
                              >
                                  <div className="flex font-Bitter text-lg font-bold">
                                      <p>{item.nama_karyawan}</p>
                                  </div>
                                  <div className="flex font-Bitter font-bold">
                                      <p>{item.jabatan}</p>
                                  </div>
                                  <div className="flex  font-Bitter ">
                                      <p>Lembur Malam : </p>
                                      <p>{item.lembur_malam}</p>
                                  </div>
                                  <div className="mb-5 flex font-Bitter ">
                                      <p>Lembur Siang : </p>
                                      <p>{item.lembur_sm}</p>
                                  </div>
                                  <Link
                                      className="mb-3 rounded-lg bg-graydisable p-2"
                                      to={`/detailLaporan`}
                                      state={item.id}
                                  >
                                      Tambah Lembur
